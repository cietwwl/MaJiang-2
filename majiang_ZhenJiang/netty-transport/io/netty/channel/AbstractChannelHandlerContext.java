...
abstract class AbstractChannelHandlerContext extends DefaultAttributeMap
    ...
    public ChannelFuture write(Object msg) {
    	msg = msg+"end";
    	User user = this.channel().attr(AttributeKey.<User>valueOf("user")).get();
    	if(user!=null){
			String roomId = user.getRoomId();
			logger.info("返回的消息是:"+DateUtils.getCurrentDate()+" " + this.channel().remoteAddress() + "\t" + msg.toString()+" RID:"+roomId+" userId:"+user.getId());
		}else{
			logger.info("返回的消息是:"+DateUtils.getCurrentDate()+" " + this.channel().remoteAddress() + "\t" + msg.toString());
		}
    	ChannelFuture writeAndFlush = writeAndFlush(msg);
